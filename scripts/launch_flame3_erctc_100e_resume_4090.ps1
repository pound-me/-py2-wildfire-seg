param(
    ...
    [string]$RunName = "flame3_erctc_partial_30e_seed200"
)
...
if ($BatchSize -ne 8) {
    throw "ERCTC continuation requires frozen physical batch 8, got: $BatchSize"
}

$MetricLines = @(Get-Content -LiteralPath $Metrics)
if ($MetricLines.Count -ne 30) {
    throw "Expected exactly 30 completed ERCTC epochs, got: $($MetricLines.Count)"
}
$LastMetric = $MetricLines[-1] | ConvertFrom-Json
if ([int]$LastMetric.epoch -ne 30) {
    throw "Expected ERCTC screening to end at epoch 30, got: $($LastMetric.epoch)"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "FLAME3 ERCTC 100-epoch continuation failed with exit code $LASTEXITCODE"
    }
}
finally {
