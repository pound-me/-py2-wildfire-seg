...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Check ERCTC zero-scale equivalence, branch gradients, AMP, "
            "parameter overhead and inference interface."
        )
    )
    parser.add_argument(
        "--config",
        ...
        default=(
            ...
            / "configs"
            / "flame3"
            / "pidnet_s_erctc_partial_30e.yaml"
        ),
    )
    parser.add_argument(
        "--root-dataset",
    ...
    if not bool(config.get("ERCTC_STANDALONE_NO_ABL_SAMF_TGM", False)):
        raise ValueError("ERCTC screening must remain a standalone structural test.")
    ...
    result = {
        ...
        "standalone_without_abl_samf_tgm": True,
    ...
    print("ERCTC engineering pipeline check passed")
